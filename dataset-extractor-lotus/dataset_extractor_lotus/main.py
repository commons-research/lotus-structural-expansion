...
def read_arg(argv):
    ...
    arg_output_path = ""
    arg_help = "{0} -o <output_path> -t <taxalevel[default=07tribe]> -m <members_of_taxalevel[default=3]> -s <samplesize_per_member[default=5]>".format(argv[0])
    

    # If argument help is given or no values for arguments are given, print the help message
    try:
        opts, args = getopt.getopt(argv[1:], "ho:t:m:s:", ["help", "output_path=", 
        "taxalevel=", "members_of_taxalevel=", "samplesize_per_member"])
    except:
        print(arg_help)
        sys.exit(2)
...
if __name__ == "__main__":
    output_path, taxalevel, members_of_taxalevel, samplesize_per_member = read_arg(sys.argv)
